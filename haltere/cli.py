...
def cmd_publish_hf(a):
    ...
    files = [('docs/hf_model_card.md', 'README.md'), ('docs/liftoff_hover.gif', 'liftoff_hover.gif'),
             ...
             ('docs/liftoff_stickfix.gif', 'liftoff_stickfix.gif'),
             ('docs/liftoff_by_sight_v05.gif', 'liftoff_by_sight_v05.gif'), ('configs/camera_seat.yaml', 'camera_seat.yaml'),
...
def main(argv=None):
    ...
    q = vs.add_parser('label', help='label every frame of the datasets with the nearest arch actually in view (labels.json)')
    ...
    q.add_argument('--augment', default='strong', choices=['light', 'strong'],
                   help='strong also varies hue, saturation, gamma, sharpness, noise and bank angle, so the detector '
                        'cannot key on the palette of one track (light is brightness and contrast only, as before)')
    ...
    q.add_argument('--ckpt', default='artifacts/ftRobust_best.pt')
