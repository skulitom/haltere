...
class GateRollout:
    def __init__(self, brain, cfg, batch=12, evaluation=False, teacher=None):
    ...
    def window(self, steps=64):
        ...
        for t in range(steps):
            ...
            if self.evaluation:
                ...
                approaching = (~self.crossed) & (self.gate[:,0]-q.pos[:,0].detach()>2.)
            ...
            if self.teacher is not None:
                with torch.no_grad():
                    ...
                    target[:,3] = (-.18*torch.atan2(relative[:,1],relative[:,0].clamp_min(.5))
                                    +.08*q.omega[:,2]).clamp(-.3,.3)
                scale = action.new_tensor([.23,.19,.084,.184])
                imitation.append(((action-target)/scale).square().mean())
            ...
            crossed,_ = aperture_crossing(q.pos.detach(),after.pos.detach(),self.gate)
        ...
        return .3*physical+3.*torch.stack(imitation).mean() if imitation else physical
...
@torch.no_grad()
def evaluate(brain,cfg,seconds=25,seed=481):
    with torch.random.fork_rng(devices=[brain.device] if brain.device.type=='cuda' else []):
        torch.manual_seed(seed)
        r = GateRollout(brain,cfg,batch=12,evaluation=True)
        ...
        return dict(seed=seed,episodes=r.B,seconds=seconds,crossings=int(r.crossed.sum()),
...
def train(args):
    ...
    rollout = GateRollout(brain,cfg,teacher=teacher)
    started = time.time()
    with (out/'training.jsonl').open('w') as log:
        for it in range(args.iters):
            if it%10==0:
                wait_if_hot(78.)
            if rollout.age>=1500 or bool(rollout.ever_crashed.any()) or bool((rollout.vs.quad.pos[:,0]>rollout.gate[:,0]+3).any()):
                rollout.reset()
            loss = rollout.window()
