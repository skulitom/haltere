...
def evaluate(checkpoint,parent,prepared,out):
    ...
    if changed-{'encoders.retina__lptc.U','encoders.retina__lptc.log_gain'}:
    ...
    for entry in m['takes']:
        ...
        goals=raw['goal'].copy();goals[ids]=labels['teacher_goal']*labels['correct']+labels['goal']*(1-labels['correct'])
        # Four independent states: parent, student, offline target, image ablation.
        ...
        outputs=[];state=brain.init_state(4)
        for i in range(n):
            obs={k:v[i:i+1].repeat(4,1) for k,v in inputs.items()}
            obs['retina'][[0,2]]=0.;obs['retina'][3]=inputs['retina'][order[i]]
        ...
        item=dict(id=entry['id'],parent_live_max_delta=retention,ticks=n,phases={})
        ...
        for name,mask in dict(arming=elapsed<5.,prefix=(correct[:,0]<.01)&(elapsed>=5.),
            ...
            errors={key:np.mean((a[mask,j]-a[mask,2])**2,axis=0).tolist()
                    for key,j in [('parent',0),('student',1),('shuffled',3)]}
