...
range when the straight segment from the camera to the point stays within VISIBLE_TUBE_M of the flown
path (the drone flew there, so nothing large can hide it), otherwise UPPER. Other sub-rays are
untouched. The same disc points constrain fan corridors (UPPER: the corridor before them is not
observed by this source). Writes <store>/labels/events.json (superset of <store>/events.json; keeps
store_event_id).
"""
...
def contact_from_telemetry(t, pos, vel, quat, t_label: float, *, search_s=(-0.4, 0.15)):
    """Contact tick near ``t_label`` (same clock as ``t``) -> dict or None.
    ...
    if len(win) == 0:
        return None
    cand = win[(acc[win] > CONTACT_ACCEL_MPS2) & (left[win] > CONTACT_UNEXPLAINED_MPS2)]
    if len(cand) == 0:
        # fall back to the strongest unexplained force in the window
        e = int(win[np.argmax(left[win])])
        found = False
    else:
        e = int(cand[0])
        found = True
    ...
    # Strongest external force within +-3 ticks of the contact tick.
    ...
    drone = P[e0]
    return dict(t_contact=float(t[e0]), drone_pos_w=drone.tolist(), speed_mps=speed, normal_w=normal.tolist(),
                normal_source=src, point_w=(drone - CONTACT_OFFSET_M * normal).tolist(),
                accel_mps2=float(acc[e]), unexplained_mps2=float(left[j]), threshold_crossed=found)
...
def unique_obstacle_key(env: str, obstacle: str, point_w, cell_m: float = 1.0) -> str:
    """Stable grouping key for repeated hits on one physical obstacle (E7): env/obstacle/rounded xy."""
    slug = env.lower().replace(' ', '-')
    kind = (obstacle or 'unknown').split(' ')[0].split('(')[0].strip().lower() or 'unknown'
    if point_w is None:
        return f'{slug}/{kind}/unlocated'
    x, y = (round(float(point_w[i]) / cell_m) * cell_m for i in (0, 1))
    return f'{slug}/{kind}-{x:.0f}-{y:.0f}'


def attach_contact_geometry(event: dict, t_phase_tel, pos, vel, quat, *, search_s=(-0.4, 0.15)) -> dict:
    """Fill point_w / normal_w / drone_pos_w / speed from 100 Hz telemetry on the CSV phase clock."""
    c = contact_from_telemetry(t_phase_tel, pos, vel, quat, event['t_phase'], search_s=search_s)
    e = dict(event)
    if c is None:
        return e
    for k in ('point_w', 'normal_w', 'drone_pos_w'):
        e[k] = [round(float(x), 3) for x in c[k]]
    if e.get('speed_mps') is None:
        e['speed_mps'] = round(c['speed_mps'], 2)
    note = (f"contact geometry from telemetry: tick {c['t_contact']:.3f} s (phase), normal from "
            f"{c['normal_source']}, external force {c['unexplained_mps2']:.0f} m/s^2, point = drone centre - "
            f"{CONTACT_OFFSET_M} m * normal")
    e['notes'] = (e.get('notes') or '') + (' | ' if e.get('notes') else '') + note
    if e.get('unique_obstacle') in (None, ''):
        e['unique_obstacle'] = unique_obstacle_key(e['env'], e['obstacle'], e['point_w'])
    return e
...
def impact_labels(event: dict, pos, quat_wb, *, flown_path=None, return_subrays: bool = False):
    """-> (grid_value, grid_kind, fan_value, fan_kind) for one frame, or None when out of view / no point.
    ...
    uv, rng, ok = world_to_pixels(p[None], o, quat_wb)
